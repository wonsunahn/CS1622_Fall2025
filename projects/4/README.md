...
Please accept Project 4 on **GitHub Classroom** using the following link: https://classroom.github.com/a/8clVc9WW
...
logs_solution/helloworld.log, if you view log_diffs/helloworld.diff.  That is
because the LLVMObject column only gets populated after code generation.  But
it should look identical otherwise.  In fact, logs/helloworld.log at this point
is the result of running codegen without the -o option, skipping the code
generation stage, since enabling it results in assertion failures.  When your
code generation stage runs without asserting, then logs/helloworld.log will
show the symbol table after code generation.  The way that this is done in the
Makefile is that it first attempts codegen with the -o option, and if it fails,
then it falls back on codegen without the -o option.
