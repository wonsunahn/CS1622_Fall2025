- [Project 4: Code Generation](#project-4-code-generation)
  ...
  * [Appendix](#appendix)
    ...
    + [Appendix B: LLVM API and LLVM Objects](#appendix-b-llvm-api-and-llvm-objects)
    + [Appendix C: LLVM IR](#appendix-c-llvm-ir)
    + [Appendix D: Debugging](#appendix-d-debugging)
    + [Appendix E: Extra Credit](#appendix-e-extra-credit)
    + [Appendix F: Obtaining the solution for Project 3](#appendix-f-obtaining-the-solution-for-project-3)
  * [Grading](#grading)
...
* outputs/ : Directory where outputs after running your parser on source files under tests/ are stored, along with the .ll LLVM IR bitcode.
* outputs_solution/ : Directory where solution outputs after running the reference parser on source files under tests/ are stored, along with the .ll LLVM IR bitcode.
* logs/, logs_solution/, log_diffs/ : Symbol table + text format AST that you already saw for Project 3 (your logs / solution logs / diffs).
* asts/, asts_solution/, ast_diffs/ : Abstract syntax tree images that you already saw for Project 2 (your asts / solution asts / diffs).
...
As mentioned above, in order to build the codegen binary, you only need to
invoke the build make target:

```
make build
```

To build and run the LLVM examples under the examples/ folder, invoke the
'examples' target:
...
To generate code for the source files under tests/ and run the generated
executables to populate the *.out files under outputs/ and diff them against
outputs_solution/ (as well as populate the *.log files under logs/ and diff
them against logs_solution), invoke the 'test' target:

```
make test
```

To generate ASTs for the source files under tests/ into the asts/ folder and
diff them against asts_solution/, invoke the 'asts' target:
...
logs/\*.log files should now contain the symbol tables and ASTs which
are the results of semantic analysis.

For example, logs/helloworld.log should contain:
...
You will notice that logs/helloworld.log still differs from
logs_solution/helloworld.log in the LLVMObject column, if you view
log_diffs/helloworld.diff.  The LLVMObject attribute is populated in the code
generation stage and stores a pointer to the LLVM object your compiler should
create for the given symbol.  LLVM objects are created when the code generation
pass encounters a definition of the symbol while traversing the syntax tree
(refer to [Appendix B: LLVM API and LLVM
Objects](#appendix-b-llvm-api-and-llvm-objects)).  There is some starter code
in codegen.cpp that creates the bare minimum of LLVM objects to have
helloworld.mjava work and produce output, so you will see that the objects for
the 'println' and 'main' functions are populated, but not those for the
'system' and 'Helloworld' classes.  The LLVMObject attribute for class symbols
will be populated eventually when you complete codegen.cpp.
...
For an example, take a look at the outputs_solution/variables.log file:
...
### Appendix B: LLVM API and LLVM Objects
...
* The llvm::Value class is an all encompassing type that can refer to any LLVM
object.  Basically all LLVM object classes that represent entities in the LLVM
IR are inherited from llvm::Value, including operators (llvm::Operator),
instructions (llvm::Instruction), local variables(llvm::AllocaInst), global
variables (llvm::GlobalVariable), constants (llvm::Constant), basic blocks
(llvm::BasicBlock), and functions (llvm::Function).  That is why the LLVMObject
attribute in the symbol table has type LLVM::Value*, as it encompasses all LLVM
IR entities that are created as a result of a definition of the symbol.

  You might ask, how can variables and instructions all be values?  Remember,
LLVM IR is in SSA (Static Single Assignment) form, so any instruction that
returns a value is associated with a unique virtual register containing a
unique value.  So values and instructions can be used interchangeably in LLVM
IR.  That is why, for example, an LLVM object representing a stack local
variable has type llvm::AllocaInst.  The llvm::AllocaInst object represents
the 'alloca' instruction in the LLVM IR that allocates stack memory for the
variable and returns the stack address.  It also represents the virtual
register containing the returned stack address of that 'alloca' instruction.
Hence, any user of that variable need to refer to the virtual register
containing the stack address for that variable, which is the llvm::AllocaInst
object.

  Likewise, llvm::Constant, llvm::Function, and llvm::BasicBlock are also
llvm::Value objects as they represent a constant value.  For example, the
llvm::Function object that is created in response to a function definition
represents the address of the function body that the code needs to jump to
whenever a user of that function calls it.  In the same way, the
llvm::BasicBlock object represents the address of the basic block which is the
jump target of any subsequent branch instruction that wants to jump to the
basic block.
...
examples_outputs/helloworld.codegen.s, the assembly file, the difference is
...
Any string prefixed by a % character on the left-hand side of assignment
...
statements, and while loops should all be there.
...
### Appendix F: Obtaining the solution for Project 3
...
points for each additional test for a total of 120 points (See [Appendix E:
Extra Credit](#appendix-e-extra-credit)).
